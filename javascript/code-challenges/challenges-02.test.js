...
const raisedToTheThird = (arr) => {
  ...
  let output = arr.map((number) => {
    //Raise each number to the 3rd power using Math.pow().
...
const forLoopTwoToThe = (arr) => {
  ...
  // Math.pow() method is used to calculate the result of raising 2 to the power of the current element of the input array.
...
const charCode = (arr) => {
  // Use the `map()` function to iterate over the input array
  // and return a new array where each element is the result
  // of the `charCodeAt()` method on the original array element.
  let charCodes = arr.map((letter) => {
    // For each element in the array, call the `charCodeAt()`
    // method to get the Unicode value of the character.
    // Save the result to a variable.
    let unicode = letter.charCodeAt(0);
    // Return the Unicode value as the new element in the
    // resulting array.
    return unicode;
  });
