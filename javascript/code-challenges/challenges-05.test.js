...
const calculateAverage = (arr) => {
  ...
  const { count, sum } = arr.reduce((acc, num) => {
    acc.count += 1;
    acc.sum += num;
    return acc;
  }, initialAccumulator);
...
const extractStats = (snorlaxData) => {
  // Solution code here...
  return snorlaxData.stats.reduce((result, stat) => {
...
const extractChildren = (arr) => {
  // Solution code here...
  const filtered = arr.filter(character => character.name.toLowerCase().includes('a'));
  return filtered.reduce((children, character) => {
    if (character.children) {
      return children.concat(character.children);
    } else {
      return children;
    }
  }, []);
